...
val df_Primary_Person_use1 = df_Primary_Person_use.withColumn("new_id",concat($"CRASH_ID",$"UNIT_NBR")).select("new_id","PRSN_ETHNICITY_ID")
val df_Units_use1 = df_Units_use.withColumn("new_id",concat($"CRASH_ID",$"UNIT_NBR")).select($"new_id",$"VEH_BODY_STYL_ID")
...
val df1 = df_Units_use.filter($"VEH_BODY_STYL_ID".isin("POLICE CAR/TRUCK", "PASSENGER CAR, 2-DOOR","PASSENGER CAR, 4-DOOR","SPORT UTILITY VEHICLE")).withColumn("new_id",concat($"CRASH_ID",$"UNIT_NBR")).select("new_id","VEH_BODY_STYL_ID")
val df2 = df_Primary_Person_use.filter($"PRSN_ALC_RSLT_ID" === "Positive").withColumn("new_id",concat($"CRASH_ID",$"UNIT_NBR")).select("new_id","DRVR_ZIP")
val df3=df1.join(df2,df1("new_id") === df2("new_id"),"inner")
val df_res6 = df3.filter($"DRVR_ZIP" !== "null").groupBy($"DRVR_ZIP").count()
val windowSpec  = Window.partitionBy().orderBy($"count".desc)
val result6 = df_res6.withColumn("row_number",row_number().over(windowSpec)).filter(($"row_number">=1) and ($"row_number"<=5)).select($"DRVR_ZIP")
display(result6)
...
val windowSpec  = Window.partitionBy().orderBy($"count".desc)
val list_of_top_10_colors_df = list_of_top_10_colors.withColumn("row_number",row_number().over(windowSpec)).filter(($"row_number">=1) and ($"row_number"<=10)).select($"VEH_COLOR_ID")
